...
**기획자가 코드 수정 없이 밸런싱할 수 있는 구조**를 만드는 게임 클라이언트 개발자입니다.

- **Unity 전투/스킬 시스템 설계** (Archer Of God)
  - `ISkill` 인터페이스 + `SkillBlocks` 모듈 조합으로 콤보 스킬 시스템 구현
  - 오브젝트 풀링으로 GC Alloc **100% 제거**, FPS **33% 향상**
  - `ScriptableObject` 기반 데이터 드리븐 설계로 기획자 독립 밸런싱 가능

- **Unity 글로벌 서비스 시스템 구축** (Dreamotion 인턴십)
  - 14개 언어 지원 폰트/UI 시스템 설계 → 언어 관련 버그 **0건**으로 데모 출시
...
> 모바일 2D 아케이드 액션 게임 | 개발기간 : 2025.09 (3일)
> 
> **📊 핵심 성과**
